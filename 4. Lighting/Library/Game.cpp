...
HRESULT InitDevice() {
    ...
    // hr= CompileShaderFromFile(L"VS.fx", "PSSolid", "ps_5_0", &solidBlob);
    // g_pd3dDevice->CreatePixelShader(solidBlob->GetBufferPointer(), solidBlob->GetBufferSize(), nullptr, &g_pPixelSolidShader);
    ...
    hr = g_pd3dDevice->CreateInputLayout(layout, 3, vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), &g_pVertexLayout);
...
void Render() {
    ...
    // [1] CBChangeEveryFrame 설정 (색상 + 월드 행렬)
    CBChangeEveryFrame cbFrame = {};
    cbFrame.World = XMMatrixTranspose(world); // HLSL에서 row_major가 아니면 transpose 필요
    cbFrame.OutputColor = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
    g_pImmediateContext->UpdateSubresource(g_pCBChangeEveryFrame, 0, nullptr, &cbFrame, 0, 0);

    // [2] CBChangeOnCameraMovement 설정 (카메라 위치 하드코딩 또는 전역 eye 값 사용)
    CBChangeOnCameraMovement cbCam = {};
    cbCam.CameraPosition = XMFLOAT4(-2.0f, 3.0f, -5.0f, 1.0f); // eye 벡터
    g_pImmediateContext->UpdateSubresource(g_pCBChangeOnCameraMovement, 0, nullptr, &cbCam, 0, 0);
    ...
    XMVECTOR v = XMLoadFloat4(&vLightPositions[1]);
    v = XMVector3Transform(v, mRotate);
    XMStoreFloat4(&vLightPositions[1], v);
...
void CleanupDevice() {
    ...
    if (solidBlob) solidBlob->Release();
}
